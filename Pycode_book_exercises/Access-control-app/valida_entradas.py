...
def read_text_yn(prompt, option_y, option_n):
        '''
        Mostra um prompt que lê uma string de 
        texto e valida se o usuário não está
        usando sequencias de escape para abortar
        ...
        while True: 
                ...
                result = result.lower()
                option_y = option_y.lower()
                option_n = option_n.lower()

                if (result != option_y) or (result != option_n):
                        print('Para sim digite: ', option_y, 'para sim ou', option_n, 'para não.')
                        continue
                break
        return result
